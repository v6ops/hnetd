...
static bool my_push_ep_id_tlv(struct tlv_buf *tb, dncp_ep_i l,
                            struct sockaddr_in6 *dst, bool always_ep_id);

static struct tlv_attr *my_push_tlv(struct tlv_buf *tb, int t, size_t len)
{
  ...
  if (tb->head)
    {
	  ...
	  // in dhcp_reply_send, but set head pointer to null explicitly
  ...
  if (!tb->head)
    {
      ...
      if (!my_push_ep_id_tlv(tb, reply->l, &reply->dst, false))
        return NULL;
    }
                            ...
                            struct sockaddr_in6 *dst, bool always_ep_id)
{
  ...
  struct tlv_attr *a = my_push_tlv(tb, DNCP_T_NODE_ENDPOINT, tl);
...
void test__push_tlv(void)
{
  ...
  hncp h = hncp_create();
  dncp o = h->dncp;
  sput_fail_unless(1==1, "__push_tlv ok");
  ...
  sput_fail_unless(l->conf.maximum_unicast_size==8952,"__push_tlv ok");

  int t=DNCP_T_REQ_NET_STATE;
  size_t len=0;
  struct tlv_attr *attr;
  //tlv_buf_init(tb, 1); /* not passed anywhere */

  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==0, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==16, "__push_tlv ok");

  t=DNCP_T_NODE_STATE;
  len=800;
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==800, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==820, "__push_tlv ok");

  t=DNCP_T_NODE_STATE;
  len=700;
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==700, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==1524, "__push_tlv ok");

  t=DNCP_T_NODE_STATE;
  len=900;
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==900, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==2428, "__push_tlv ok");

  t=DNCP_T_NODE_STATE;
  len=1000;
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==3432, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==4436, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==5440, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==6444, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==7448, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==1016, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==2020, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==3024, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==4028, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==5032, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==6036, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==7040, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==8044, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==1016, "__push_tlv ok");
  ...
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==10000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==10016, "__push_tlv ok");
  dncp_reply_send(&reply);
  tb->head=NULL;
  ...
  fprintf (stderr,"max %li\n",l->conf.maximum_unicast_size);
  sput_fail_unless(l->conf.maximum_unicast_size==1232,"__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==0, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==16, "__push_tlv ok");
  ...
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==300, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==316, "__push_tlv ok");

  t=DNCP_T_NODE_STATE;
  len=400;
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==400, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==720, "__push_tlv ok");
  ...
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==400, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==416, "__push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==400, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==820, "__push_tlv ok");
...
void test_my_push_tlv(void)
{
  /*
   * Make sure the my_push_tlv function works as we want 
   */
  struct sockaddr_in6 *src=NULL;
  struct sockaddr_in6 *dst=NULL;
  ...
  hncp h = hncp_create();
  dncp o = h->dncp;
  sput_fail_unless(1==1, "_my_push_tlv ok");

  // set id
  int nilen = h->ext.conf.node_id_length;
  char *nibuf = alloca(nilen);
  int i;
  for (i = 0; i < nilen; i++)
    nibuf[i] = i % 256;
  //my_dncp_set_own_node_id(o, nibuf);
  ...
  fprintf (stderr,"max %li\n",l->conf.maximum_unicast_size);
  sput_fail_unless(l->conf.maximum_unicast_size==8952,"_my_push_tlv ok");
  ...
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==0, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==16, "_my_push_tlv ok");

  t=DNCP_T_NODE_STATE;
  len=800;
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==800, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==820, "_my_push_tlv ok");

  t=DNCP_T_NODE_STATE;
  len=700;
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==700, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==1524, "_my_push_tlv ok");

  t=DNCP_T_NODE_STATE;
  len=900;
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==900, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==2428, "_my_push_tlv ok");

  t=DNCP_T_NODE_STATE;
  len=1000;
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==3432, "_my_push_tlv ok");
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==4436, "_my_push_tlv ok");
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==5440, "_my_push_tlv ok");
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==6444, "_my_push_tlv ok");
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==7448, "_my_push_tlv ok");
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==8452, "_my_push_tlv ok");
  attr=_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==1000, "__push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==1016, "__push_tlv ok");
  ...
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==10000, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==10016, "_my_push_tlv ok");
  dncp_reply_send(&reply);
  tb->head=NULL;
  ...
  l->conf.maximum_unicast_size=1280-40-8;
  fprintf (stderr,"max %li\n",l->conf.maximum_unicast_size);
  sput_fail_unless(l->conf.maximum_unicast_size==1232,"_my_push_tlv ok");
  //tlv_buf_init(tb, 13); /* not passed anywhere */
  tlv_buf_free(tb);
  tb->head=NULL;
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  ...
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==300, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==1124, "_my_push_tlv ok");

  t=DNCP_T_NODE_STATE;
  len=400;
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==400, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==416, "_my_push_tlv ok");
  ...
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==400, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==1224, "_my_push_tlv ok");
  attr=my_push_tlv(tb,t,len);
  fprintf (stderr,"total %i len %i\n",tlv_len(tb->head),tlv_len(attr));
  sput_fail_unless(tlv_len(attr)==400, "_my_push_tlv ok");
  sput_fail_unless(tlv_len(tb->head)==416, "_my_push_tlv ok");
...
//  sput_run_test(hncp_ext);
  ...
  sput_run_test(test_my_push_tlv);
